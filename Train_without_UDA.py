...
log_txt_path = './log_labels={}.txt'.format(args.labels)
...
train_writer = tf.summary.FileWriter('./logs/train_without_UDA_labels={}'.format(labels))
train_ops = [train_op, loss_op, s_loss_op, l2_reg_loss_op, accuracy_op, train_summary_op]
...
for iter in range(1, MAX_ITERATION + 1):
    ...
    batch_data = [batch_s_image_list, batch_s_label_list]
    ...
    _, loss, s_loss, l2_reg_loss, accuracy, summary = sess.run(train_ops, feed_dict = _feed_dict)
    ...
    if iter % 100 == 0:
        ...
        log_print('[i] iter = {}, loss = {:.4f}, supervised_loss = {:.4f}, l2_reg_loss = {:.4f}, accuracy = {:.2f}%, train_time = {}sec'.format(iter, loss, s_loss, l2_reg_loss, accuracy, train_time), log_txt_path)
...
saver.save(sess, './model/wihtout_UDA_labels={}.ckpt'.format(labels))
