...
log_txt_path = './log_labels={}@SU_ratios={}.txt'.format(args.labels, su_ratios)
...
train_writer = tf.summary.FileWriter('./logs/train_with_UDA_labels={}@SU_ratios={}'.format(labels, su_ratios))
train_ops = [train_op, loss_op, s_loss_op, u_loss_op, l2_reg_loss_op, accuracy_op, train_summary_op]

train_threads = []
for i in range(NUM_THREADS):
    train_thread = Teacher(labeled_data_list, unlabeled_data_list, ratio = su_ratios, debug = False, max_data_size = 20)
    train_thread.start()
    ...
    while not find:
        for train_thread in train_threads:
            if train_thread.ready:
                find = True
                batch_data = train_thread.get_batch_data()        
                break
...
saver.save(sess, './model/UDA_labels={}@SU_ratios={}.ckpt'.format(labels, su_ratios))

for train_thread in train_threads:
    train_thread.end = True
    train_thread.join()
