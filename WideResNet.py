...
def bn_relu_conv(x, filters, kernel_size, strides, padding, is_training, scope, bn = True, activation = True, use_bias = False):
    with tf.variable_scope(scope):
        if bn: x = tf.layers.batch_normalization(x, training = is_training, momentum = BATCH_NORM_DECAY, epsilon = BATCH_NORM_EPSILON, name = 'bn')
...
def WideResNet(input_var, is_training, depth = 28, widen_factor = 2, init_channel = 32, reuse = False, getter = None):
    ...
    with tf.variable_scope('Wider-ResNet-28', reuse = reuse, custom_getter = getter):
        # 1. input = 32x32
        x = input_var / 255.
        x = (x - transform.MEANS) / transform.STDS

        x = bn_relu_conv(x, channels[0], [3, 3], 1, 'same', is_training, 'conv1')

        # block 1 : 32x32
        x = ResNetBlock(x, is_training, n, 1, channels[1], 'Block1'); # print(x)

        # block 2 : 16x16
        x = ResNetBlock(x, is_training, n, 2, channels[2], 'Block2'); # print(x)
        
        # block 3 : 8x8
        x = ResNetBlock(x, is_training, n, 2, channels[3], 'Block3'); # print(x)

        x = tf.layers.batch_normalization(x, training = is_training, momentum = BATCH_NORM_DECAY, epsilon = BATCH_NORM_EPSILON, name = 'bn')
